...
function createAgent(options: { model?: string; provider?: string; mode?: string; hybrid?: boolean }) {
...
program
  ...
  .option('--hybrid', 'Enable hybrid mode (text block parsing for non-tool-use models)')
  .option('--tui', 'Enable TUI mode (requires ink/React)')
  .action(async (options) => {
    try {
      const { agent, providerName, config, skillRegistry } = createAgent(options);
      ...
      console.log('\x1b[36m║          HalfCopilot v0.0.1                      ║\x1b[0m');
      console.log('\x1b[36m║   Multi-model Agent Framework CLI                ║\x1b[0m');
      console.log('\x1b[36m╚══════════════════════════════════════════════════╝\x1b[0m');
      console.log(`\x1b[32mProvider:\x1b[0m ${providerName}`);
      console.log(`\x1b[32mModel:\x1b[0m ${options.model ?? config.defaultModel ?? 'mimo-v2.5-pro'}`);
      console.log(`\x1b[32mMode:\x1b[0m ${options.mode ?? 'auto'}`);
      if (options.hybrid) {
        console.log(`\x1b[33mHybrid Mode: Enabled\x1b[0m`);
      }
      console.log(`\x1b[33mType your message and press Enter. Type 'exit' to quit.\x1b[0m\n`);
      ...
      const ask = () => {
        rl.question('\x1b[32m❯ \x1b[0m', async (input) => {
          ...
          if (matchedSkills.length > 0) {
            console.log(`\x1b[35m[Skills matched: ${matchedSkills.map(s => s.name).join(', ')}]\x1b[0m`);
          }
          ...
          try {
            ...
            console.log(`\x1b[31m[Error: ${err instanceof Error ? err.message : err}]\x1b[0m`);
          }
...
program
  ...
  .action(() => {
    try {
      ...
      const providerRegistry = new ProviderRegistry();
      providerRegistry.createFromConfig(config);
      console.log('\x1b[36mAvailable providers:\x1b[0m', providerRegistry.list().join(', '));
    } catch (err) {
      console.error('\x1b[31mError:\x1b[0m', err instanceof Error ? err.message : err);
    }
  });
    ...
    try {
      ...
      for (const skill of skillRegistry.list()) {
        ...
        if (skill.triggers && skill.triggers.length > 0) {
          console.log(`  Triggers: ${skill.triggers.map(t => t.value).join(', ')}`);
        }
        console.log();
      }
    } catch (err) {
...
program
  ...
  .action(() => {
    ...
    try {
      ...
      const providerRegistry = new ProviderRegistry();
      providerRegistry.createFromConfig(config);
      console.log(`\x1b[32m✓\x1b[0m Providers: ${providerRegistry.list().join(', ')}`);

      console.log(`\x1b[32m✓\x1b[0m Default provider: ${config.defaultProvider}`);
      console.log(`\x1b[32m✓\x1b[0m Default model: ${config.defaultModel}`);

      const toolRegistry = new ToolRegistry();
      const builtinTools = createBuiltinTools();
      builtinTools.forEach(t => toolRegistry.register(t));
      console.log(`\x1b[32m✓\x1b[0m Built-in tools: ${toolRegistry.list().join(', ')}`);

      const skillRegistry = new SkillRegistry();
      const builtinSkills = createBuiltinSkills();
      builtinSkills.forEach(s => skillRegistry.register(s));
      console.log(`\x1b[32m✓\x1b[0m Skills: ${skillRegistry.list().map(s => s.name).join(', ')}`);
